...
public class UserServiceImpl implements UserService {
	...
	public Map<String, Object> sendAuthCode(String email) {
		 ...
		 * 
		 * 1. 구글 로그인 2. [Google 계정] - [보안] 1) [2단계 인증] - [사용] 2) [앱 비밀번호] (1) 앱 선택 : 기타
		 * (2) 기기 선택 : Windows 컴퓨터 (3) 생성 버튼 : 16자리 앱 비밀번호를 생성해 줌(이 비밀번호를 이메일 보낼 때 사용)
		 */
	...
	public void join(HttpServletRequest request, HttpServletResponse response) {
		...
		// DB로 보낼 UserDTO 만들기
		UserDTO user = UserDTO.builder().id(id).pw(pw).name(name).gender(gender).email(email).mobile(mobile)
				.birthyear(birthyear).birthday(birthday).postcode(postcode).roadAddress(roadAddress)
				.jibunAddress(jibunAddress).detailAddress(detailAddress).extraAddress(extraAddress).agreeCode(agreeCode)
				.build();
	...
	public void keepLogin(HttpServletRequest request, HttpServletResponse response) {
		 ...
		 * 로그인 유지를 체크한 경우
		 * 
		 * 1. session_id를 쿠키에 저장해 둔다. (쿠키명 : keepLogin) 2. session_id를 DB에 저장해 둔다.
		 ...
		 * 로그인 유지를 체크하지 않은 경우
		 * 
		 * 1. 쿠키 또는 DB에 저장된 정보를 삭제한다. 편의상 쿠키명 keepLogin을 제거하는 것으로 처리한다.
		 */
		...
		if (keepLogin != null) {
			...
			UserDTO user = UserDTO.builder().id(id).sessionId(sessionId)
					.sessionLimitDate(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24 * 15)) // 현재타임스탬프 + 15일에
																										// 해당하는 타임스탬프
					.build();
	...
	public void modifyPassword(HttpServletRequest request, HttpServletResponse response) {
		...
		if (pw.equals(loginUser.getPw())) {
			try {
				...
				out.println("<script>");
				out.println("alert('현재 비밀번호와 동일한 비밀번호로 변경할수없습니다.');");
				out.println("history.back();");
				out.println("</script>");
				out.close();
...
//		int userNo = ((UserDTO)request.getSession().getAttribute("loginUser")).getUserNo();
		...
		UserDTO user = UserDTO.builder().userNo(userNo).pw(pw).build();
		...
		try {
			...
			if (result > 0) {
				...
				out.println("<script>");
				out.println("alert('비밀 번호 수정이 실패했습니다.');");
				out.println("history.back();");
				out.println("</script>");
	...
	@Override
	@Transactional
	public void resotreUser(HttpServletRequest request, HttpServletResponse response) {
		...
		if (pw.equals(sleepUser.getPw())) {
			...
			try {
				...
				if (insertCount > 0 && deleteCount > 0) {
					out.println("<script>");
					out.println("alert('휴먼 계정이 복구 되었습니다. 휴면 계정 활성화를 위해 곧바로 로그인을 해주세요');");
					...
					out.println("<script>");
					out.println("alert('휴먼계정이 복구되지 않았습니다');");
					out.println("history.back();");
					out.println("</script>");
				}
			} catch (Exception e) {
